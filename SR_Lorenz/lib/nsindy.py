...
def proximal(w, lam=0.1, eta=0.1):
	...
	tmp = abs(w) - lam * eta
	#with torch.no_grad():
	#	w.weight = torch.clamp(tmp, min=0)
	w.weight = torch.clamp(tmp, min=0)
	#v = torch.nn.functional.normalize(wadj, dim=1) * alpha[:, None, :]
	#w.data = v.view(-1, func.dims[0])

def train_ista(odefunc, lr, nepoch, niterbatch, lMB, nMB, odeint_method, GL_reg, train_data, val_data, t, ckpt_path, fig_save_path):
	train_data = torch.tensor(train_data)
	val_data = torch.utils.data.DataLoader(torch.tensor(val_data),batch_size=50)
	...
	for itr in range(nepoch):
		...
		for i in range(niterbatch):
			...
			optimizer.step()
			proximal(odefunc.C.weight, GL_reg, 0.01)
